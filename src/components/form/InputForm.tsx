...
export default function ContinuousSlider({getPachinko}:{getPachinko: (requestParams: GetPachinkoRequestParam) => Promise<void>}): JSX.Element {
  ...
  return (
    <FormArea>
      ...
      <SliderArea>
        <InputSlider title={"低確率状態での大当たり確率[1/n]"} value={normal} max={400} min={100} step={0.1} changeHandler={changeNormal} key={0} />
        <InputSlider title={"高確率状態での大当たり確率[1/n]"} value={koukaku} max={400} min={10} step={0.1} changeHandler={changeKoukaku} key={1} />
        <InputSlider title={"高確率状態突入率[%]"} value={kakuhen} max={100} min={0} step={0.1} changeHandler={changeKakuhen} key={2} />
        <InputSlider title={"高確率状態継続率[%]"} value={keizoku} max={100} min={0} step={0.1} changeHandler={changeKeizoku} key={3} />
      </SliderArea>
