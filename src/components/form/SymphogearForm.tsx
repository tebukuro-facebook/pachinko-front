...
const SymphogearForm = ({getSymphogear}:{getSymphogear: (requestParams: SymphogearParam) => Promise<void>}): JSX.Element => {
  ...
  return (
    <FormArea>
      ...
      <SliderArea>
        <InputSlider title={"特図2での大当たり確率[1/n]"} value={normal} max={400} min={100} step={0.1} changeHandler={changeNormal} key={0} />
        <InputSlider title={"特図2での小当たり確率[1/n]"} value={koukaku} max={400} min={10} step={0.1} changeHandler={changeKoukaku} key={1} />
        <InputSlider title={"特図1当せん時の電サポ回数[回]"} value={tokuzu1} max={100} min={0} step={0.1} changeHandler={changeTokuzu1} key={2} />
        <InputSlider title={"特図2当せん時の電サポ回数[回]"} value={tokuzu2} max={100} min={0} step={0.1} changeHandler={changeTokuzu2} key={3} />
